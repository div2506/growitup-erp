...
export default function LoginPage() {
  const { user, setUser, setMyEmployee, loading } = useAuth();
  ...
  useEffect(() => {
    ...
    if (params.get("error")) setErrorMsg(decodeURIComponent(params.get("error")));
  }, [user, loading, navigate]);

  const handleGoogleSuccess = async (tokenResponse) => {
    // tokenResponse.access_token from implicit flow → fetch userinfo
    setSigningIn(true);
    setErrorMsg("");
    try {
      ...
      // Send userinfo to our backend to create session
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        {errorMsg && (
          <div className="mb-4 px-4 py-3 bg-red-500/15 border border-red-500/30 rounded-lg text-red-400 text-sm text-center" data-testid="login-error">
            {errorMsg}
          </div>
        )}
        ...
        <button
          ...
          disabled={signingIn}
          className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white text-gray-800 rounded-lg font-medium hover:bg-gray-100 transition-colors duration-200 shadow-lg disabled:opacity-70 disabled:cursor-wait"
        >
          {signingIn ? (
